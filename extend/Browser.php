...
https://github.com/ajinabraham/simple-php-browser-detection/blob/master/browser-detect.php
*/
class Browser
{
    ...
    */
    ...
    @return 
    {
      ...
      "session_id": "6sk5kihlk4c2pun2vm1mg22lm1"
    }
    */
    public static function fetchInfo()
    {
        ...
        $arr['browser_version'] = $browser->getVersion();
        $arr['platform'] = $browser->getPlatform();
        $arr['http_user_agent'] = $_SERVER['HTTP_USER_AGENT'];
        $arr['session_id'] = session_id();
        return $arr;
    }
    
    public static function isAndroidForChromeAndEdge()
    {
        $arr = \Browser::fetchInfo();
        // _json($arr);
        if(
    ...
    {
        $Browser = new \Browser();
        $is1 = $Browser->checkBrowseriPhone();
        $is2 = $Browser->checkBrowseriPad();
        $is3 = $Browser->checkBrowseriPod();
        if($is1 || $is2 || $is3){
            return true;
        }
        return false;
    ...
    public static function mobile_platform() 
    { 
        $platform = '';
    ...
    const PLATFORM_APPLE = 'Apple Mac';
    ...
    {
        if(
            ($this->checkForFacebookIos() == true)
            || ($this->checkForFacebookAndroid() == true)
        ){
            return true;
        }
        return false;
    ...
    protected function checkForFacebookAndroid()
    {
        if (stristr($this->_agent, 'FBAV')) {
            $this->setFacebook(true);
            return true;
        }
        return false;
